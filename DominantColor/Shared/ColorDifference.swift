...
internal actor ColorDifference {
    ...
    ///
    nonisolated internal func CIE76SquaredColorDifference(_ lab1: simd_float3, lab2: simd_float3) -> Float {
    ...
    internal func CIE94SquaredColorDifference(
        ...
        return { (lab1:simd_float3, lab2:simd_float3) -> Float in
            ...
            let (C1, C2) = (self.C(a1, b: b1), self.C(a2, b: b2))
            let ΔC = C1 - C2
    ...
    internal func CIE2000SquaredColorDifference(
        ...
        kC: Float = 1,
        kH: Float = 1
    ) async -> (_ lab1:simd_float3, _ lab2:simd_float3) async -> Float {
        return { (lab1:simd_float3, lab2:simd_float3) async -> Float in
            let (L1, a1, b1) = lab1.unpack()
            let (L2, a2, b2) = lab2.unpack()
            ...
            let (C1p, C2p) = (self.C(a1p, b: b1), self.C(a2p, b: b2))
            let ΔCp = C2p - C1p
            let Cbp = (C1p + C2p) / 2
