...
internal actor ColorSpaceConverter {
    ...
    internal let D65Tristimulus = simd_float3(5.047, 100.0, 108.883)

    internal func IN_RGBToLAB(_ gVector: simd_float3) -> simd_float3 {
    ...
    internal func IN_LABToRGB(_ gVector: simd_float3) -> simd_float3 {
    ...
    private func RGBToSRGB(_ rgbVector: simd_float3) -> simd_float3 {
    #if os(iOS)
        return rgbVector
    #elseif os(OSX)
    ...
    private func SRGBToRGB(_ srgbVector: simd_float3) -> simd_float3 {
    #if os(iOS)
        return srgbVector
    #elseif os(OSX)
    ...
    private func SRGBToLinearSRGB(_ srgbVector: simd_float3) -> simd_float3 {
    ...
    private func LinearSRGBToSRGB(_ lSrgbVector: simd_float3) -> simd_float3 {
        func f(_ c: Float) -> Float {
            if (c <= 0.0031308) {
                return c * 12.92
            } else {
    ...
    private func XYZToLAB(_ xyzVector: simd_float3, _ tristimulus: simd_float3) -> simd_float3 {
    ...
    private func LABToXYZ(_ labVector: simd_float3, _ tristimulus: simd_float3) -> simd_float3 {
        func f(_ t: Float) -> Float {
            if (t > (6.0 / 29.0)) {
                return powf(t, 3.0)
            } else {
